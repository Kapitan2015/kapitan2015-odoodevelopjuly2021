...
class AccountFollowupReport(models.AbstractModel):
	...
	@api.model
	def send_email(self, options, force_send=True, raise_exception=False):
		#NEVIO DE CORREO AL CLIENTE PARA SEGUIMINETO DE FACTURAS ADEUDADAS
		...
		if ms_email:
			...
			email = self.env['mail.mail'].create({
				...
				'email_from': (self.env.user.company_id.name) + ' <' + ms_email + '>' or '', #self.env.user.email or '',
				'email_to': email,
				'body': msg,
				# 'force_email': True,
			})
			...
			if force_send:
				logging.info("entro en force_send: ")
				email.send(raise_exception=raise_exception)
				# logging.info("mail id: " + str(mail.id))
			return email.id 
		else:
